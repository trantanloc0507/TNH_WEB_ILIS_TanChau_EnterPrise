...
const isArrayCoordinate = (obj)=>Array.isArray(obj) && obj.every(v=>!isNaN(v));

const ProcessRings = (obj)=>{
    if (obj.every(isArrayCoordinate))
        return obj.map(e=>{
            return {
                lat:e[1],
                lng:e[0]
...
const parse = (wkt)=> {
    ...
    function coords () {
        ...
        return list.length ? ProcessRings(list) : null;
    }
